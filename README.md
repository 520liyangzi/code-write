...
不同文件可以使用相同的规则编号。例如 `Java编码规范.md` 和 `Java安全规范.md` 都包含 `G.COM.01` 时，系统会自动生成两个内部唯一编号：
...
页面仍会显示文档中的原始编号，搜索 `G.COM.01` 也能同时找到这两条规则。只有发生冲突的编号才会增加文件名命名空间；没有冲突的已有编号保持不变。
...
审批区默认只显示“待处理”规则。单条保存或批量批准成功后，已经处理的卡片会立即从主列表中收起，不会继续占满页面。

需要复查时，点击列表上方绿色的“查看已批准规则（数量）”；直接批准和“修改后接受”的规则都会显示。查看完成后点击“收起已批准规则”返回待处理工作区。决定状态下拉框仍可单独查看直接批准、修改后接受、已拒绝或全部规则；“清除筛选”会回到默认的待处理规则视图。
...
如果整批文档已经由可信流程确认，不需要逐条点击：

1. 先抽查每份文档的规则结构、级别、描述和正反例；
2. 点击审批区右上角的“一键批准全部待处理（数量）”；
3. 在确认框中核对规则总数和文档数；
4. 点击确认，等待“全部批准完成”。

该操作用一次请求原子批准全部待处理规则，300 条规则也不需要逐条保存。它不会覆盖已经批准、修改后接受或拒绝的规则；如果页面中有尚未保存的单条决定，会先要求保存，避免误覆盖。
...
不会。系统会在冲突编号前自动增加来源文件名，例如 `Java安全规范::G.CMT.02`，并把 `G.CMT.02` 保留为文档原始编号。审批、激活和数据库使用内部唯一编号；页面展示和检索同时支持原始编号。

如果同一范围内的两个文件不仅规则编号相同，连文件名也相同，上传阶段仍会拒绝，以防一份文件静默覆盖另一份文件。请先给文件使用可区分的名称。
